...
- `custodianRef` and other payload-level identity fields are out of scope for this ADR; per-command-type validation of the asserting party (`fromCustodianRef`) is handled in `packages/auth`/`packages/transport-http`, and counterparty-binding (`toCustodianRef`/`registerCocoaLot.custodianRef`) is deliberately not built absent a validated acceptance-workflow need (DR-012, `docs/execution/ACCELERATED_AUTONOMOUS_DECISION_REGISTER.md`).
