...
describe("SW1-04 service runtime", () => {
  it("loads bounded configuration and rejects invalid ports", () => {
    ...
    expect(loadServiceConfig({ ORIGINOS_AUTH_MODE: "oidc", ORIGINOS_OIDC_ISSUER: "https://id.example", ORIGINOS_OIDC_AUDIENCE: "originos", ORIGINOS_OIDC_JWKS_URI: "https://id.example/jwks", ORIGINOS_OIDC_AGENCY_REFS_CLAIM: "custom_agency_refs", ORIGINOS_OIDC_AUTHORITY_REFS_CLAIM: "custom_authority_refs", ORIGINOS_OIDC_CUSTODIAN_REFS_CLAIM: "custom_custodian_refs" }).authentication).toEqual({ mode: "oidc", issuer: "https://id.example", audience: "originos", jwksUri: "https://id.example/jwks", agentRefsClaim: "originos_agent_refs", requiredScope: "originos:commands", agencyRefsClaim: "custom_agency_refs", authorityRefsClaim: "custom_authority_refs", custodianRefsClaim: "custom_custodian_refs" });
  ...
  it("runs the complete Cocoa workflow through HTTP and preserves it across graceful restart", async () => {
    ...
    try {
      ...
      const browserCompletion = createCocoaProcessingCompletionEnvelope({ completionId: "ui-browser-completion", transformationRef: "originos:transformation-ui-browser-processing-transformation", processorRef: "originos:processor-1", outputQuantityKg: 700, accepted: true, consequence: "processing-yield-recorded", agentRef: "originos:merchant-1", agencyRef: "originos:agency-cocoa-procurement", authorityRef: "originos:authority-cocoa-procurement", purposeRef: "originos:purpose-conforming-cocoa", evidenceRef: "originos:evidence-cocoa-processing-report", attributionRule: "originos:attribution-direct-agent" });
      const browserCompletionResponse = await fetch(`${service.baseUrl}/v2/commands`, { method: "POST", headers: { ...authHeaders, "content-type": "application/json", "idempotency-key": browserCompletion.commandId }, body: JSON.stringify(browserCompletion) });
      expect(browserCompletionResponse.status).toBe(201);
      const persistedBrowserCompletion = await fetch(`${service.baseUrl}/v2/records/originos%3Acompletion-ui-browser-completion`, { headers: authHeaders }).then((response) => response.json()) as { ok: boolean; value: { content: { transformationRef: string; outputQuantityKg: number; yieldPercent: number; completionStatus: string } } };
      expect(persistedBrowserCompletion).toMatchObject({ ok: true, value: { content: { transformationRef: "originos:transformation-ui-browser-processing-transformation", outputQuantityKg: 700, yieldPercent: 700 / 750 * 100, completionStatus: "completed" } } });
      const processedLot = createProcessedCocoaLotEnvelope({ processedLotId: "ui-browser-processed-lot", completionRef: "originos:completion-ui-browser-completion", agentRef: "originos:merchant-1", agencyRef: "originos:agency-cocoa-procurement", authorityRef: "originos:authority-cocoa-procurement", purposeRef: "originos:purpose-conforming-cocoa", evidenceRef: "originos:evidence-cocoa-processing-report", attributionRule: "originos:attribution-direct-agent" });
      const processedLotResponse = await fetch(`${service.baseUrl}/v2/commands`, { method: "POST", headers: { ...authHeaders, "content-type": "application/json", "idempotency-key": processedLot.commandId }, body: JSON.stringify(processedLot) });
