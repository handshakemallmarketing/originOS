...
export const createOriginHttpHandler = (application: OriginApplication, receipts: CommandReceiptStore, options: OriginHttpOptions): OriginHttpHandler => async (request, response) => {
  try {
    ...
    if (request.method === "POST" && url.pathname === "/v2/commands") {
      ...
      // Only fromCustodianRef (the party asserted to be relinquishing custody) is bound — toCustodianRef and registerCocoaLot's custodianRef name a counterparty rather than assert the caller's own identity (see issue #10).
