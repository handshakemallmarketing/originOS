...
export const createOriginHttpHandler = (application: OriginApplication, receipts: CommandReceiptStore, options: OriginHttpOptions): OriginHttpHandler => async (request, response) => {
  try {
    ...
    if (request.method === "POST" && url.pathname === "/v2/commands") {
      ...
      if (principal.permittedAgencyRefs && !principal.permittedAgencyRefs.includes(envelope.agencyRef)) return audited(403, { ok: false, error: { code: "ORIGINOS_AUTH_003_AGENCY_BINDING_DENIED", message: "Authenticated principal is not bound to the declared Agency" } }, { event: "command-request", commandId: envelope.commandId, commandType: envelope.command.commandType, statusCode: 403, outcome: "agency-binding-rejected" }, { "cache-control": "no-store" });
      if (principal.permittedAuthorityRefs && !principal.permittedAuthorityRefs.includes(envelope.authorityRef)) return audited(403, { ok: false, error: { code: "ORIGINOS_AUTH_004_AUTHORITY_BINDING_DENIED", message: "Authenticated principal is not bound to the declared Authority" } }, { event: "command-request", commandId: envelope.commandId, commandType: envelope.command.commandType, statusCode: 403, outcome: "authority-binding-rejected" }, { "cache-control": "no-store" });
      // Only fromCustodianRef (the party asserted to be relinquishing custody) is bound — toCustodianRef and registerCocoaLot's custodianRef name a counterparty rather than assert the caller's own identity. Recipient-binding is deliberately not built absent a validated acceptance-workflow need (DR-012, docs/execution/ACCELERATED_AUTONOMOUS_DECISION_REGISTER.md).
      if (principal.permittedCustodianRefs && envelope.command.commandType === "transferCustody" && typeof envelope.command.payload.fromCustodianRef === "string" && !principal.permittedCustodianRefs.includes(envelope.command.payload.fromCustodianRef)) return audited(403, { ok: false, error: { code: "ORIGINOS_AUTH_005_CUSTODIAN_BINDING_DENIED", message: "Authenticated principal is not bound to the declared Custodian relinquishing custody" } }, { event: "command-request", commandId: envelope.commandId, commandType: envelope.command.commandType, statusCode: 403, outcome: "custodian-binding-rejected" }, { "cache-control": "no-store" });
